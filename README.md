# Huda Math Lab — Cloudflare Pages + D1
...
npx wrangler d1 create huda-math-lab
```
...
```bash
npx wrangler d1 execute huda-math-lab --local  --file=./schema.sql
npx wrangler d1 execute huda-math-lab --remote --file=./schema.sql
```
...
variable name `DB`, database `huda-math-lab`. The variable name has to be `DB` —
